...
2. An [OpenAI API account](https://platform.openai.com/) with API billing enabled and a project API key. ChatGPT billing is separate from API billing.
...
Keep the bot token, OpenAI key, Google client secret, database password, and `APP_SECRET_KEY` private. Never add `.env` to Git.
...
1. Sign in at [OpenAI's API platform](https://platform.openai.com/).
2. Configure [API billing](https://help.openai.com/en/articles/9039756), create/select a project, and set a budget alert suitable for you.
3. Create a [project API key](https://platform.openai.com/api-keys), save it privately, and confirm that the model you choose is available to that project. The repository's default `OPENAI_PRIMARY_MODEL` is `gpt-6-astra`; you can change it to another model that supports the app's structured outputs and image inputs.

This key becomes `OPENAI_API_KEY`. Model calls are a separate usage cost from hosting and Google Calendar.
...
2. In the provider firewall, allow SSH from your IP. Outbound HTTPS must work for Telegram, Google, and OpenAI. You do not need public port 8000.
...
6. Send `Should I spend 1000 MYR on a laptop?` to try the decision council. This makes several model calls, so it costs more than a simple reminder.

For problems, run `docker compose logs --tail 100 app` (or `sudo docker compose logs --tail 100 app` on Linux). Common causes are an incorrect Telegram ID, invalid model/API key, unavailable model, missing Google test user, or a Google redirect URI mismatch. If the bot does not respond, check `docker compose ps` and the readiness URL first.
...
- Check OpenAI usage and the VPS bill after the first week. Calendar API standard use should remain within the free threshold for a personal bot; [Google publishes the current quota and pricing rules](https://developers.google.com/workspace/calendar/api/guides/quota).
